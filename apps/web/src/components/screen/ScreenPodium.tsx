...
export default function ScreenPodium({ podium }: ScreenPodiumProps) {
  ...
  useEffect(() => {
    ...
    // 3+ players: Sequential podium reveals
    const t1 = setTimeout(() => {
      setCeremonyStep(1);
      soundManager.playRevealChime(true);
    }, 1200);
  ...
  return (
    <div className="relative flex flex-col h-full text-white p-8 md:p-12 bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] from-blue-900 via-[#1e3a5f] to-black justify-between select-none overflow-hidden">
      ...
      {/* Celebratory confetti elements when 1st place is revealed */}
      ...
      {podium.length === 1 ? (
        <div className="flex items-end justify-center h-[480px] md:h-[520px] w-full max-w-md mx-auto my-auto z-10">
          <div className="flex flex-col items-center w-full z-20 min-h-[440px] justify-end">
            {first && showFirst ? (
        ...
        <div className="flex items-end justify-center gap-8 h-[480px] md:h-[520px] w-full max-w-3xl mx-auto my-auto z-10">
          {/* Segundo Lugar (Silver) */}
          <div className="flex flex-col items-center w-1/2 z-10 min-h-[360px] justify-end">
            {second && showSecond ? (
          ...
          <div className="flex flex-col items-center w-1/2 z-20 min-h-[440px] justify-end">
            {first && showFirst ? (
              <div className={`w-full flex flex-col items-center ${!reducedMotion ? 'animate-[podiumSlideUp_0.8s_ease-out_both]' : ''}`}>
                <div className="text-5xl md:text-6xl mb-1 animate-bounce motion-reduce:animate-none">
                  🏆
